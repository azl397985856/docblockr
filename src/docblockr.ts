...
function includeTypes() {
    return vs.workspace.getConfiguration().get("doc.includeTypes", true);
}

function inferTypes() {
    return vs.workspace.getConfiguration().get("doc.inferTypesFromNames", false);
}

function enableHungarianNotationEvaluation() {
    return vs.workspace.getConfiguration().get("doc.enableHungarianNotationEvaluation", false);
}

export class DocBlockr implements vs.Disposable {
    ...
    private _emitDescriptionHeader(sb: utils.SnippetStringBuilder) {
        if (vs.workspace.getConfiguration().get("doc.includeDescriptionTag", false)) {
    ...
    private _emitMemberOf(sb: utils.SnippetStringBuilder, parent: ts.Node) {
        let enabledForClasses = parent.kind === ts.SyntaxKind.ClassDeclaration && vs.workspace.getConfiguration().get("doc.includeMemberOfOnClassMembers", true);
        let enabledForInterfaces = parent.kind === ts.SyntaxKind.InterfaceDeclaration && vs.workspace.getConfiguration().get("doc.includeMemberOfOnInterfaceMembers", true);
